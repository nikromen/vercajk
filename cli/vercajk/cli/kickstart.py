...
def _get_kickstart_template_path() -> Path:
    ...
    return ConfigManager.get_config().repo_path / "files" / "image_template.ks.j2"
...
def _render_kickstart_template(tags: list[str], output_file: str) -> None:
    ...
    if not template_path.exists():
        print(
            f"Error: Kickstart template not found at {template_path}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def kickstart(tag: tuple[str], output: str):
    ...
    print(
        f"3. Add 'inst.ks=file:/path/to/{os.path.basename(output)}' to the kernel parameters",
    )
